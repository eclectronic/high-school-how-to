...
export class ImagePickerComponent implements OnInit, OnChanges {
  ...
  protected onUploadComplete(result: UploadedImageResult): void {
    this.pendingUploadFile.set(null);
    // Reload the grid so the new asset appears, then auto-select by URL
    this.api.list(this.search, this.page).subscribe({
      next: (p) => {
        this.assets.set(p.content);
        this.totalPages = p.totalPages;
        const uploaded = p.content.find((a) => a.url === result.url);
        if (uploaded) {
          this.select(uploaded);
        }
      },
    });
  }
