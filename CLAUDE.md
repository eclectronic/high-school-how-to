...
- **Tags are always called "topics" in admin UI**: The data model uses `Tag`/`tag` internally, but any admin-facing label, button, heading, error message, toast, or dialog copy must say "topic" / "topics" — never "tag" / "tags". This applies to both frontend and backend error messages surfaced to the UI.
...
- **Image uploads (local dev)**: In Docker dev mode (`SPRING_PROFILES_ACTIVE=docker`), `LocalStorageService` is active. Uploaded images are written to `./media/uploads/{subfolder}/` (mounted into the API container at `/workspace/media`) and returned as `/media/uploads/...` URLs served by the Angular dev server. In prod, `S3StorageService` uploads to `s3://highschoolhowto/uploads/` and returns CloudFront URLs.
...
| `s3://highschoolhowto/uploads/` | `./media/uploads/` | Admin-uploaded images (thumbnails, content images). |
