...
/**
 * Integration tests for human-readable filename logic in ImageUploadController.
 *
 * Verifies that:
 * - Uploading with title="Study Tips" produces a URL containing "study-tips".
 * - Uploading the same title again returns 409 with existingUrl in body.
 */
class ImageUploadFilenameIntegSpec extends BaseIntegrationSpec {
    ...
    def "POST /api/admin/images/upload with title='Study Tips' produces URL containing 'study-tips'"() {
        given:
        def admin = createAdminUser("filename-admin-1@example.com")
        def token = jwtService.generateAccessToken(admin)
        def expectedUrl = "https://cdn.example.com/images/study-tips.jpeg"
        def expectedThumbUrl = "https://cdn.example.com/thumbs/study-tips.jpeg"

        when(storageService.generateFilename(anyString(), anyString(), anyString())).thenReturn("study-tips.jpeg")
        when(storageService.upload(any(byte[].class), anyString(), anyString(), anyString())).thenReturn(expectedUrl)
        // thumbnail upload returns separate URL
        when(storageService.upload(any(byte[].class), anyString(), anyString(), org.mockito.ArgumentMatchers.eq("thumbs"))).thenReturn(expectedThumbUrl)

        def file = new MockMultipartFile("file", "photo.jpeg", "image/jpeg", TINY_JPEG)

        when:
        def response = mockMvc.perform(
                MockMvcRequestBuilders.multipart("/api/admin/images/upload")
                        .file(file)
                        .param("title", "Study Tips")
        ...
        (body['url'] as String).contains("study-tips")
    }

    def "POST /api/admin/images/upload with duplicate title returns 409 with existingUrl"() {
        given:
        def admin = createAdminUser("filename-admin-2@example.com")
        def token = jwtService.generateAccessToken(admin)
        def existingUrl = "https://cdn.example.com/images/study-tips.jpeg"
        def conflictKey = "images/study-tips.jpeg"

        // The real default method will call objectExists — we stub it to throw conflict
        when(storageService.generateFilename(anyString(), anyString(), anyString()))
                .thenThrow(new FilenameConflictException(conflictKey, existingUrl))
        ...
        response.status == 409
        def body = objectMapper.readValue(response.contentAsString, Map)
        body['existingUrl'] == existingUrl
        body['error'] == 'Filename conflict'
    }

    def "POST /api/admin/images/upload returns 401 without auth"() {
        given:
        def file = new MockMultipartFile("file", "photo.jpeg", "image/jpeg", TINY_JPEG)
